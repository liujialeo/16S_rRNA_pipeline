...
#!/bin/bash
    ...
    pwd
    db=/c/public
    wd=/g/bioinformation/06_others/wangbin/16s
    cd ${wd}
...
### 5.1 生成特征表 Creat Feature table
    ...
    time ${db}/win/vsearch --usearch_global temp/filtered.fa --db result/raw/otus.fa \
    	--otutabout result/raw/otutab.txt --id 0.9 --threads 20
...
### 5.2 物种注释-去除质体和非细菌/古菌并统计比例(可选) Remove plastid and non-Bacteria
    ...
    time ${db}/win/vsearch --sintax result/raw/otus.fa --db ${db}/usearch/silva_16s_v123.fa \
      --tabbedout result/raw/otus.sintax --sintax_cutoff 0.1
